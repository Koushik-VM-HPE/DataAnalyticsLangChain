...
@tool
def list_clusters(limit: int, filter: Dict[str, any] = None) : # type: ignore
    """
    List 'limit' number clusters from the DynamoDB table based on given filter.
 
    **Parameters:**
    - `limit` (Optional[int]): Maximum number of clusters to return.  
        - Set to `None` to retrieve all clusters.  
        - Example: `10` (returns up to 10 clusters).  
    - `filter` (Optional[Dict[str, any]]): Key-value pairs used as filtering criteria.  
        ...
        - Example: `{"available": Attr("available").eq("true")}` retrieves clusters that have the attribute set to true.
    
    **Return:**
    List of cluster items.

    **Notes:**  
    - This function interacts with DynamoDB and applies filters dynamically.  
    - The agent should specify meaningful filter conditions to refine results.  
    """
    try:
        ...
        print(f"########### filter expresion is {filter}")

        items = DynamoDBUtils.scan(table_name="devex_ocm_intg", filter=filter, limit=limit)
...
tools = [list_clusters, search_weather_tool]
tool_names = ["list_clusters", "search_weather_tool"]
